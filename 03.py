...
st.write("Choose either to upload a video file or use the live webcam for real-time vehicle detection and tracking.")
...
def process_video_with_tracking(video_path):
    ...
    return process_video_stream(cap)

# Function to process webcam stream with vehicle tracking
def process_webcam_with_tracking():
    cap = cv2.VideoCapture(0)  # 0 is the default webcam
    return process_video_stream(cap)

# Common function to process a video stream (used by both video files and webcam)
...
# If user selects to use the webcam
    ...
    # Process webcam stream with tracking
    output_video_path, vehicle_counts = process_webcam_with_tracking()
    
    # Display cumulative vehicle counts
    st.write("**Vehicle Count in Live Webcam** (Real-Time Tracking):")
    for vehicle, count in vehicle_counts.items():
        st.write(f"{vehicle}: {count}")
